...
describe('Dessert entries — meal_type=dessert', () => {
  it('returns 201 when dessert is attached to a dinner parent', async () => {
    mockState.plan = { id: 'plan-1', week_start: '2026-03-01' }
    mockState.parentEntryMealType = 'dinner'
    const res = await entriesPOST(makeReq('POST', 'http://localhost/api/plan/entries', {
      week_start: '2026-03-01',
  ...
  it('returns 201 when dessert is attached to a lunch parent', async () => {
    mockState.plan = { id: 'plan-1', week_start: '2026-03-01' }
    mockState.parentEntryMealType = 'lunch'
    const res = await entriesPOST(makeReq('POST', 'http://localhost/api/plan/entries', {
      week_start: '2026-03-01',
  ...
  it('returns 400 when dessert is attached to a breakfast parent', async () => {
    mockState.plan = { id: 'plan-1', week_start: '2026-03-01' }
    mockState.parentEntryMealType = 'breakfast'
    const res = await entriesPOST(makeReq('POST', 'http://localhost/api/plan/entries', {
      week_start: '2026-03-01',
  ...
  it('returns 400 when dessert is attached to a snack parent', async () => {
    mockState.plan = { id: 'plan-1', week_start: '2026-03-01' }
    mockState.parentEntryMealType = 'snack'
    const res = await entriesPOST(makeReq('POST', 'http://localhost/api/plan/entries', {
      week_start: '2026-03-01',
  ...
  it('returns 400 when dessert is missing parent_entry_id', async () => {
    mockState.plan = { id: 'plan-1', week_start: '2026-03-01' }
    const res = await entriesPOST(makeReq('POST', 'http://localhost/api/plan/entries', {
      week_start: '2026-03-01',
      date: '2026-03-01',
